...
class TestSettings(PropertyGroup):
    ...
    bpy.types.Scene.fps_base = FloatProperty(
        update=updateFPSBase, min=0.05, max=2, default=1, precision=2)
    bpy.types.Scene.fps_base_enum = \
        EnumProperty(default='3', update=updateFPSBaseEnum,
                     items=[
                         ("1", "0.25x", "a"),
                         ("2", "0.50x", "s"),
                         ("3", " 1x ",  "d"),
    ...
    bpy.types.Scene.rest_layers_opacity = FloatProperty(
        update=updateGPLayerOpacity, min=0, max=1, default=1)
    ...
    bpy.types.Scene.show_overlays = BoolProperty(
        default=False, update=dummyUpdate)
    bpy.types.Scene.show_gizmo = BoolProperty(
        default=False, update=dummyUpdate)
    bpy.types.Scene.show_t_menu = BoolProperty(
        default=False, update=dummyUpdate)
    bpy.types.Scene.show_n_menu = BoolProperty(
        default=False, update=dummyUpdate)
    bpy.types.Scene.show_bool_toggle = BoolProperty(
        default=False, update=dummyUpdate)
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname):
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            if item:
                ...
                split.label(text="(" + str(item.frame_preview_start) +
                            " - " + str(item.frame_preview_end) + ")")
