...
class PIE_MT_KEY_CONDITIONS(Menu):
    ...
    def draw(self, context):
        ...
        pie.operator(
            "wm.call_menu", text="Sequencer Conds").name = "VIEW3D_MT_SEQUENCER_CONDITIONS"
        pie.operator(CN, text="").cond = ""
        pie.operator(CN, text="").cond = ""
...
class PIE_MT_S(Menu):
    ...
    def draw(self, context):
        ...
        if M in (MHE, CVE):
            pie.operator(
                MT, text="Tools").name = "VIEW3D_MT_COMMON_MODELING_TOOLS"
        if M in (MHS):
            pie.operator(
                MT, text="Some Brushes").name = "VIEW3D_MT_SOME_BRUSHES"
        if M in (GPE):
        ...
        if M in (GPS, GPW, GPP):
            pie.operator(
                MT, text="Select Layers").name = "VIEW3D_GP_LAYERS_MT_2"
        # ------------------------   BOTTOM-RIGHT   ------------------------- #
        if M in (OBJ, MHE, GPE, ARE, ARP, CVE, LCE):
            pie.operator(MT, text="Cursor").name = "VIEW3D_MT_CURSOR_POSITION"
        if M in (MHS):
            pie.operator(
                MT, text="Detailing").name = "VIEW3D_MT_DYNTOPO_DETAILING"
...
class VIEW3D_MT_SHADING_PIE(Menu):
    ...
    def draw(self, context):
        ...
        pie.operator("wm.call_menu",
                     text="Rendered").name = "VIEW3D_MT_EEVEE_OR_CYCLES"
        pie.operator("wm.call_panel", text="MatCap").name = "VIEW3D_PT_matcap"
        pie.operator("wm.call_panel",
                     text="Background").name = "VIEW3D_PT_background"
        ...
        pie.operator("wm.call_menu",
                     text="Options").name = "VIEW3D_MT_SHADING_OPTIONS"
...
class VIEW3D_MT_ANIMATION_PIE(Menu):
    ...
    def draw(self, context):
        ...
        pie.operator("wm.call_menu", text="")
        pie.operator("wm.call_menu",
                     text="Playback").name = "VIEW3D_MT_ANIMATION_PLAYBACK"

        if context.area.type == 'DOPESHEET_EDITOR':
            pie.operator(
                "aaa.toggle_prop", text="test").prop = "context.space_data.dopesheet.show_only_selected"
        else:
            pie.operator("wm.call_menu", text="")

        pie.operator("wm.call_menu", text="")
        pie.operator("wm.call_menu",
                     text="dgsasg").name = "VIEW3D_MT_ANIMATION_OPS"
        pie.operator("wm.call_menu",
                     text="About Frames").name = "VIEW3D_MT_ABOUT_FRAMES"
        pie.operator("wm.call_menu", text="")
...
class VIEW3D_MT_TIMELINE_PIE(Menu):
    ...
    def draw(self, context):
        ...
        if bpy.context.area.type == 'DOPESHEET_EDITOR':
            pie.operator("wm.call_menu",
                         text="Transform").name = "VIEW3D_TIMELINE_TRANSFORM"
        elif bpy.context.area.type == 'GRAPH_EDITOR':
            pie.operator(
                "wm.call_menu", text="Transform").name = "VIEW3D_GRAPH_EDITOR_TRANSFORM"
        else:
        ...
        pie.operator("wm.call_menu",
                     text="Workspace").name = "VIEW3D_MT_WORKSPACE"
        pie.operator("wm.call_menu", text="").name = ""
        pie.operator("wm.call_menu", text="").name = ""
...
class VIEW3D_MT_GP_BRUSHES_PIE(Menu):
    ...
    def draw(self, context):
        ...
        if M == MHT:
            pie.operator("wm.call_menu", text="").name = ""
            pie.operator(
                "aaa.set_prop", text="Draw").prop_set = "context.tool_settings.image_paint.brush = bpy.data.brushes['TexDraw']"
            pie.operator("wm.call_menu", text="").name = ""
            pie.operator("wm.call_menu", text="").name = ""
            pie.operator("wm.call_menu", text="").name = ""
            pie.operator(
                "aaa.set_prop", text="Erase").prop_set = "context.tool_settings.image_paint.brush = bpy.data.brushes['TexErase_1']"
            pie.operator("wm.call_menu", text="").name = ""
            pie.operator("wm.call_menu", text="").name = ""

        if M == GPP:
            for i in range(1, 9):
                exec("pie.operator(ST, text=SC.brush_name_" +
                     str(i)+").name=SC.brush_name_"+str(i))
